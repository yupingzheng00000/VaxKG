...
class DualRanker(nn.Module):
    """
    Dual-head ranking model for both vaccine→adjuvant and disease→adjuvant queries.
    
    Design principles:
    - Shared encoder: Reuse PyGHeteroEncoder + optional APPNP for all node types
    - Independent heads: Separate Bilinear scorers to avoid cross-task interference
    - Mechanism-aware: Optional compatibility scoring between disease and adjuvant mechanisms
    - Backward compatible: Vaccine head preserves exact behavior of single-head baseline
    """
    def __init__(self, hidden_dim: int, mech_in_dim: Optional[int] = None, gamma: float = 0.3) -> None:
        super().__init__()
        # Head 1: Vaccine→Adjuvant scorer (legacy, must preserve performance)
        self.vax_head = nn.Bilinear(hidden_dim, hidden_dim, 1)
        
        # Head 2: Disease→Adjuvant scorer (NEW)
        self.dis_head = nn.Bilinear(hidden_dim, hidden_dim, 1)
        
        # Optional: Mechanism-aware compatibility (novel contribution)
        self.gamma = gamma
        if mech_in_dim is not None and mech_in_dim > 0:
            self.mech_proj = nn.Linear(mech_in_dim, hidden_dim)
            self.mech_bil = nn.Bilinear(hidden_dim, hidden_dim, 1)
        else:
            self.mech_proj = None
            self.mech_bil = None
    
    def score_vax(self, h_vax: Tensor, h_adj: Tensor) -> Tensor:
        """Vaccine→Adjuvant compatibility score (batch mode)."""
        # h_vax: [B, H], h_adj: [B, K, H]
        # Expand vaccine embeddings to match adjuvant candidates
        if h_vax.dim() == 2 and h_adj.dim() == 3:
            h_vax_expanded = h_vax.unsqueeze(1).expand_as(h_adj)  # [B, K, H]
            scores = self.vax_head(h_vax_expanded, h_adj).squeeze(-1)  # [B, K]
        else:
            # Fallback: pairwise scoring
            scores = self.vax_head(h_vax, h_adj).squeeze(-1)
        return scores
    
    def score_dis(self, h_dis: Tensor, h_adj: Tensor, mech_vec: Optional[Tensor] = None) -> Tensor:
        """Disease→Adjuvant compatibility score (optionally mechanism-aware)."""
        # Base compatibility score
        if h_dis.dim() == 2 and h_adj.dim() == 3:
            h_dis_expanded = h_dis.unsqueeze(1).expand_as(h_adj)  # [B, K, H]
            base_score = self.dis_head(h_dis_expanded, h_adj).squeeze(-1)  # [B, K]
        else:
            base_score = self.dis_head(h_dis, h_adj).squeeze(-1)
        
        # Add mechanism-aware compatibility if enabled
        if mech_vec is not None and self.mech_bil is not None:
            mech_emb = self.mech_proj(mech_vec)  # [B, K, H] or [K, H]
            if h_dis.dim() == 2 and mech_emb.dim() == 2:
                h_dis_expanded = h_dis.unsqueeze(1).expand(h_dis.size(0), mech_emb.size(0), -1)
                mech_emb_expanded = mech_emb.unsqueeze(0).expand_as(h_dis_expanded)
                mech_score = self.mech_bil(h_dis_expanded, mech_emb_expanded).squeeze(-1)
            else:
                mech_score = self.mech_bil(h_dis_expanded, mech_emb).squeeze(-1)
